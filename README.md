...
| **보고서 매니저 (Report Manager)** | **HCX-007 (Reasoning)** | 정해진 양식에 따른 문서 작성 및 **지시 이행** 능력이 우수하며, 자가 교정 로직으로 정확도를 보완합니다. |
| **라우터 (Router)** | **HCX-003 (Standard)** | 사용자 의도를 파악하여 신속하게 분기 처리를 수행하는 밸런스 모델이 적합합니다. |
| **분석 필드 선택기 (Field Selector)** | **HCX-003 (Standard)** | 복잡한 **JSON 스키마**를 엄격하게 준수하는 구조적 출력 안정성을 고려했습니다. |
| **검색기 (Retriever)** | **HCX-DASH-002 (Light)** | 단순 데이터 조회 및 SQL 변환 업무를 빠르게 처리하여 시스템 **응답 속도**를 최적화합니다. |
