...
# =============================================================
    ...
    with ai_tab:
        ...
        with st.expander("🔎 키워드 사례 검색 (빠른 검색)", expanded=False):
            st.caption("대화하기 전에 관련 사례를 먼저 훑어보세요. 키워드를 복사해 아래 AI 채팅에 붙여넣으면 더 깊이 분석할 수 있어요.")
            search_query = st.text_input("검색어 입력", placeholder="예: 경징계 재무/회계/계약 종합감사", label_visibility="collapsed", key="keyword_search")
            if search_query:
                ...
                if not preview.empty:
                    st.markdown(f"**📋 키워드 매칭 결과** ({len(preview)}건)")
