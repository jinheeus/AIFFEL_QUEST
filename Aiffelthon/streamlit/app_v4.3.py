            ...
            if i + j < len(rows_list):
                ...
                with col:
                    ...
                    st.markdown(
                        ...
                        f'<span class="tag tag-red">📌 {disp}</span>'
                        f'<span class="tag tag-blue">🔍 {atype}</span>'
                        f'<span class="tag tag-orange">💰 {pen_label}</span>'
                        f'{org_cat_tag}'
                        f'</div>',
                        unsafe_allow_html=True
                    )
                    # 복사 가능한 키워드
                    kw = f"{org} {disp} {risk} {atype} 패널티({pen_label})"
...
BLACKEDGE_NEWS = [
    ...
    {"date": "2026-02-05", "title": "무역보험공사 환변동보험, 스왑포인트 불공정·환투기 방치",
     "summary": "일반형 보장환율 산정 방식이 기업에 불리하게 설계",
     "url": "https://blackedge.news/news/6323", "tag": "재무/회계/계약"},
    {"date": "2026-02-05", "title": "무역보험공사 간부, 기재부 사무관에 '숙박권' 제공해 청탁금지법 위반",
     "summary": "직무 관련 기획재정부 공무원에게 휴양소 숙박권 제공 적발",
     "url": "https://blackedge.news/news/6320", "tag": "윤리/부패/비위"},
    {"date": "2026-02-05", "title": "무역보험공사, 임금체불 기업 64곳에 255억 보증 등 수출신용보증 심사 부실",
     "summary": "보증 제한사유 심사와 사후관리 부실 지적",
     "url": "https://blackedge.news/news/6318", "tag": "재무/회계/계약"},
    {"date": "2026-02-05", "title": "'서울시설공단 따릉이' 개인정보 대규모 유출, 개보위와 경찰 조사 착수",
     "summary": "서울시 공공자전거 서비스 개인정보 유출 사건",
     "url": "https://blackedge.news/news/6302", "tag": "정보보안/IT"},
    {"date": "2026-02-05", "title": "감사원, 2026년 공공기관 자체감사활동 심사 착수",
     "summary": "공공기관 자체감사활동에 대한 연례 심사 개시",
     "url": "https://blackedge.news/news/6207", "tag": "사업/운영/성과"},
    {"date": "2026-02-05", "title": "외부 해킹이 주범인데 내부 통제만 강화…공공 개인정보 관리의 허점 드러나",
     "summary": "외부 해킹 대응 vs 내부 통제 강화 간 불균형 지적",
     "url": "https://blackedge.news/news/6263", "tag": "정보보안/IT"},
    {"date": "2026-02-05", "title": "KOICA, 직무정지 임원에 성과급 지급·퇴직금 과다 산정…인건비 관리 총체적 부실",
     "summary": "직무정지 기간 성과급 지급, 퇴직금 과다 산정 등 복합 부실",
     "url": "https://blackedge.news/news/6270", "tag": "인사/채용/복무"},
    ...
    {"date": "2026-02-05", "title": "교포수입자 총액한도 '절반 제한'…손해율은 더 낮아졌는데 차별 규정은 15년째 유지",
     "summary": "교포수입자 대상 총액한도 차별적 규제 15년간 유지",
     "url": "https://blackedge.news/news/6322", "tag": "재무/회계/계약"},
]
...
if menu == "Home":
    ...
    if not df.empty:
        ...
        src_parts = [f"{s} {c:,}건" for s, c in site_counts.items()]
        org_cat_cnt = df['org_category'].nunique()
        st.caption(f"📌 데이터: {' · '.join(src_parts)} | 기관유형 {org_cat_cnt}종 | 전체 {len(df):,}건")
        ...
        st.caption("최신 감사 결과 및 주요 이벤트 (기관별 최대 3건) · 클릭 시 원문 이동")

        recent_base = df.dropna(subset=['date']).sort_values('date', ascending=False)
        if not recent_base.empty:
            diversified = recent_base.groupby('agency_category').head(3).sort_values('date', ascending=False).head(10)
            # download_url이 있는 경우 하이퍼링크 카드로 렌더링
            display_rows = list(diversified.head(10).iterrows())
...
# =============================================================
    ...
    if df.empty:
        ...
        kc3.metric("📁 유형 수",f"{fdf['audit_report_type'].nunique()}개"); kc4.metric("📅 기간",f"{ds.strftime('%Y.%m')}~{de.strftime('%Y.%m')}")
        st.divider()
...
키워드 클라우드는 TF-IDF 기반 키워드 빈도 분석으로 별도 산출됩니다.
        ...
        with tab2:
            ...
            vt_site = vt_f2.selectbox("데이터 소스", vt_site_opts, key="vt_site")
