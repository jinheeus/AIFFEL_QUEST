...
if menu == "Home":
    ...
    if not df.empty:
        ...
        if org_scores:
            ...
            # --- 수정된 부분: 전체 데이터(df) 기준의 Global Max 값을 고정값으로 산출 ---
            # (매번 연산하는 것을 방지하기 위해 전체 데이터의 근사 최대치를 하드코딩하거나 df 기준으로 한 번만 추출합니다)
            global_max_cnt = df['agency_category'].value_counts().max() # 단일 기관 최대 지적 건수 (예: LH 1214건)
            global_max_sev = 10.0  # 처분 강도 최대치는 중징계(10점)로 고정
            global_max_rep = 100.0 # 반복 비율 최대치는 100%로 고정
            
            osd['cnt_n'] = (osd['cnt'] / global_max_cnt * 100) if global_max_cnt > 0 else 0
            osd['sev_n'] = (osd['sev'] / global_max_sev * 100) if global_max_sev > 0 else 0
            osd['rep_n'] = (osd['rep'] / global_max_rep * 100) if global_max_rep > 0 else 0
            # --------------------------------------------------------------------------
