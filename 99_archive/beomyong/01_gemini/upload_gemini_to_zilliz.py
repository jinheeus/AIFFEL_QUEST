...
def load_env_variables():
    """
    .env 파일에서 필요한 환경 변수들을 로드하고 유효성을 검사합니다.
    """
    load_dotenv()
    ...
    if not google_api_key:
        raise ValueError("환경 변수 'GOOGLE_API_KEY'가 .env 파일에 설정되지 않았습니다.")
    if not zilliz_uri:
    ...
    print("✅ 필요한 환경 변수들을 성공적으로 로드했습니다.")
    return google_api_key, zilliz_uri, zilliz_token
...
def main():
    """
    데이터를 로드하고 Gemini로 임베딩하여 Zilliz Cloud에 업로드하는 메인 함수
    """
    COLLECTION_NAME = "audit_cases_gemini_v1"
    try:
        _, zilliz_uri, zilliz_token = load_env_variables()
    ...
    # 2. 임베딩 모델 초기화
    print("\n🧠 Gemini 임베딩 모델을 초기화합니다 (models/text-embedding-004)...")
    try:
        embeddings = GoogleGenerativeAIEmbeddings(model="models/text-embedding-004")
    except Exception as e:
        print(f"🚨 임베딩 모델 초기화 중 에러가 발생했습니다: {e}")
        print("   'langchain-google-genai' 패키지가 올바르게 설치되었는지 확인해주세요.")
        return

    # 3. Zilliz에 데이터 업로드 (배치 처리)
    print(f"\n☁️ Zilliz Cloud에 데이터를 업로드합니다 (Collection: '{COLLECTION_NAME}')...")
    
    batch_size = 128  # Gemini 임베딩은 배치 처리에 더 효율적이므로 배치 크기를 늘립니다.
    ...
    try:
        ...
        vector_store = Zilliz.from_documents(
            ...
            auto_id=True,
            drop_old=True # 기존 컬렉션이 있다면 삭제하고 새로 생성
        )
        print("  - 첫 번째 배치 완료.")
        time.sleep(1) # API 속도 제한 및 서버 부하를 위한 최소 대기
        ...
        print("   - 'pymilvus', 'langchain-google-genai' 패키지가 설치되었는지 확인해주세요.")
